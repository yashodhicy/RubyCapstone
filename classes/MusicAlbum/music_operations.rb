...
class MusicAlbumOperations
  ...
  def list
    ...
    if @albums.empty?
      ...
      @albums.each do |album|
        album_details = "Album id: #{album.id}, genre: #{album.genre.name}, published date: #{album.publish_date},"
  ...
  def add_album(genre_operations)
  ...
  def print_album_details(album, genre)
    album_details = "Album id: #{album.id}, genre: #{genre.name}, published date: #{album.publish_date},"
    album_details2 = "on Spotify: #{album.on_spotify}  archived: #{album.archived}"
    puts "#{album_details} #{album_details2}"
  end
end
